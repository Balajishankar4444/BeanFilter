        ...
        <div className="mx-auto max-w-5xl px-6 text-center sm:px-8">
          ...
          <div className="animate-fade-up delay-4 mx-auto mt-10 max-w-2xl">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2 text-xs text-stone-600">
              <span className="font-medium text-stone-500">Popular searches:</span>
              {['Netherlands', 'Germany', 'United Kingdom', 'United States', 'France'].map((country) => (
                <button
                  key={country}
                  onClick={() => handlePopularSearch(country)}
                  className={`rounded-full border px-3 py-1 text-xs font-medium transition-all duration-200 ${
                    selectedCountry === country
                      ? 'border-stone-800 bg-stone-900 text-[#E8DCC8]'
                      : 'border-stone-300 bg-[#F3EBDD]/60 text-stone-800 hover:border-stone-400 hover:bg-[#F3EBDD]'
                  }`}
                >
                  {country}
                </button>
        ...
        <div className="mb-10 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-semibold tracking-tight text-stone-950 sm:text-4xl">
              Find Coffee Roasters Around the World
            </h2>
            <p className="mt-2 text-base text-stone-600">
              Explore specialty coffee roasters by country and city.
            </p>
          </div>
          <button
            onClick={() => handlePopularSearch('ALL')}
            className="inline-flex items-center gap-1.5 text-sm font-medium text-stone-800 hover:text-stone-950 transition-colors"
          >
          ...
          {[
            {
              ...
              flag: '🇺🇸',
              desc: 'World-class roasters across Brooklyn, Portland, Chicago and West Coast.',
              count: initialRoasters.filter((r) => r.country === 'United States').length,
            },
            {
              country: 'United Kingdom',
              flag: '🇬🇧',
              desc: 'Iconic specialty roasters in London, Edinburgh and Manchester.',
              count: initialRoasters.filter((r) => r.country === 'United Kingdom').length,
            },
            {
              country: 'Netherlands',
              flag: '🇳🇱',
              desc: 'Specialty coffee roasters across Amsterdam, Rotterdam, Utrecht and more.',
              count: initialRoasters.filter((r) => r.country === 'Netherlands').length,
            },
            {
              country: 'Germany',
              flag: '🇩🇪',
              desc: 'Pioneering roasters in Berlin, Hamburg, Munich and beyond.',
              count: initialRoasters.filter((r) => r.country === 'Germany').length,
            },
            {
              country: 'France',
              flag: '🇫🇷',
              desc: 'Artisanal specialty coffee roasters in Paris and Lyon.',
              count: initialRoasters.filter((r) => r.country === 'France').length,
            },
            {
              country: 'Italy',
              flag: '🇮🇹',
              desc: 'Craft specialty and modern espresso roasters across Milan and Florence.',
              count: initialRoasters.filter((r) => r.country === 'Italy').length,
            },
          ].map((loc) => (
            <div
              key={loc.country}
              onClick={() => handlePopularSearch(loc.country)}
            ...
            >
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {loc.count > 0 && (
                    <span className="rounded-full bg-stone-300/70 px-3 py-1 text-xs font-semibold text-stone-700">
                      {loc.count} {loc.count === 1 ? 'Roaster' : 'Roasters'}
                    </span>
                  )}
                </div>
              ...
              <div className="mt-6 flex items-center text-xs font-semibold text-stone-800 group-hover:translate-x-1 transition-transform">
